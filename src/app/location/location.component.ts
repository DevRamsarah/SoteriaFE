  ...
  ngOnInit(): void {
    ...
    this.fire.collection('postSite', ref => ref.where("ClientName", "==", JSON.parse(localStorage.getItem('CurentUser'))[0].fname)).valueChanges()
      .subscribe((PostSite: any) => {
        ...
        this.data.push({
          Longitude: PostSite[count].Longitude,
          Latitude: PostSite[count].Latitude,
          ClientName: PostSite[count].ClientName,
          PsLocation: PostSite[count].PsLocation,
          ClientAddress: PostSite[count].ClientAddress
        })
        count++
      })
